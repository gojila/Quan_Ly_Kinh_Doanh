	...
	public class ConvertFont
	{
		...
		private bool isSpecialChar(string s, bool isUNI)
		{
			...
			else
			{
				...
				{
					if (string.Compare(s, array3[i], true) == 0)
					{
						result = true;
						return result;
					}
		...
		private FontIndex getCode(string str)
		{
			...
			else
			{
					...
					else
					{
						for (int j = 7; j > 0; j--)
						{
							...
							for (int k = 0; k < 7; k++)
							{
								...
								if (j != 3 || k == 1)
								{
									if ((k != 3 && k != 2 && k != 5 && k != 4) || j <= 2)
									{
										int l = 1;
										while (l < 135)
										{
											if (text == this.Code[l, k])
											{
												if (!this.isSpecialChar(text, k == 5 || k == 6))
												{
													result = (FontIndex)int.Parse(this.Code[0, k]);
													return result;
												}
												num = k;
												break;
				...
				result = (FontIndex)((num >= 0) ? int.Parse(this.Code[0, num]) : -1);
			}
			return result;
		}
